...
export const admin_login = createAsyncThunk(
    ...
    async (info, { rejectWithValue, fulfillWithValue }) => {
        try {
            const { data } = await axios.post(`${base_url}/api/admin-login`, info, { withCredentials: true })
...
export const seller_login = createAsyncThunk(
    ...
    async (info, { rejectWithValue, fulfillWithValue }) => {
        try {
            const { data } = await axios.post(`${base_url}/api/seller-login`, info, { withCredentials: true })
            localStorage.setItem('accessToken', data.token)
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
...
export const seller_register = createAsyncThunk(
    ...
    async (info, { rejectWithValue, fulfillWithValue }) => {
        try {
            console.log(info)
            const { data } = await axios.post(`${base_url}/api/seller-register`, info, { withCredentials: true })
            localStorage.setItem('accessToken', data.token)
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
...
export const profile_image_upload = createAsyncThunk(
    ...
    async (image, { rejectWithValue, fulfillWithValue,getState }) => {
        ...
        try {
            const { data } = await axios.post(`${base_url}/api/profile-image-upload`, config)
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }

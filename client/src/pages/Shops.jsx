...
// Define a list of colors for filtering
const colors = [
...
const Shops = () => {
    ...
    }, [state.values[0], state.values[1], category, selectedColors, rating, pageNumber, sortPrice])
    ...
    return (
        <div>
            <Headers />
            <section className='bg-[url("https://shining-tn.vercel.app/images/banner/shop.gif")] h-[220px] mt-6 bg-cover bg-no-repeat relative bg-left'>
                ...
                <div className='w-[85%] md:w-[90%] sm:w-[90%] lg:w-[90%] h-full mx-auto'>
                    ...
                    <div className='w-full flex flex-wrap'>
                        <div className={`w-3/12 md-lg:w-4/12 md:w-full pr-8 ${filter ? 'md:h-0 md:overflow-hidden md:mb-6' : 'md:h-auto md:overflow-auto md:mb-0'}`}>
                            ...
                            <div className='py-2'>
                                {
                                    categorys.map((c, i) => <div className='flex justify-start items-center gap-2 py-1' key={i}>
                                        <input checked={category === c.name} onChange={(e) => setCategory(e.target.checked ? c.name : '')} type="checkbox" id={c.name} />
                            ...
                            <div className='py-2'>
                                <h2 className='text-3xl font-bold mb-3 text-slate-600'>Colors</h2>
                                <div className='flex flex-wrap gap-2'>
                                    {colors.map(color => (
                                        <button
                                            key={color.value}
                                            className={`w-10 h-10 rounded-full ${selectedColors.includes(color.value) ? 'border-2 border-blue-500' : 'border-2 border-transparent'} cursor-pointer`}
